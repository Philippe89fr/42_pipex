...
/*   Updated: 2025/01/10 10:01:18 by prambaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av, char **env)
{
	...
	pipex.av = av;
	pipex.env = env;
	if (pipex.ac < 5 || (!ft_strncmp(pipex.av[1], "here_doc", 9)
			&& pipex.ac < 6))
		return (0);
	if (ft_strncmp(pipex.av[1], "here_doc", 9))
	{
	...
	{
		pipex.pid = malloc((pipex.ac - 2) * sizeof(pid_t));
		if (!pipex.pid)
			return (0);
		ft_bonus(pipex);
	}
